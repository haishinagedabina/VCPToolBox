...
- 存在 → 提取 `name`、`topics`、`tone`、`voice`、`blacklist`、`theme`、`cover_style`、`author`、`content_style_preferences`、`writing_persona_default`
...
根据选题特征和内容风格, 从 `personas/` 目录动态选择最合适的写作人格:
- `midnight-friend` → 随笔、个人感悟、深夜聊天风格
- `industry-observer` → 行业趋势、深度分析
- `sharp-journalist` → 热点评论、观点输出、反驳主流
- `warm-editor` → 温暖治愈、人文关怀、生活方式
- `cold-analyst` → 数据驱动、产品评测、方法论

选择时综合考虑: 选题类型、目标受众、情绪基调、框架类型。`style.yaml` 的 `writing_persona_default` 只是偏好起点, 当选题特征明显指向另一个人格时, 以选题为准。
